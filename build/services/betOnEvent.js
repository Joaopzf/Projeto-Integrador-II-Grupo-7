...
exports.betOnEvent = void 0;
const mysql_1 = __importDefault(require("../db/mysql")); // Importando o pool de conexão
const betOnEvent = async (email, eventId, betAmount) => {
    try {
        // Verifica se o apostador existe
        const [userRows] = await mysql_1.default.execute("SELECT * FROM users WHERE email = ?", [email]);
        const user = userRows[0];
        if (!user) {
            throw new Error("Apostador não encontrado.");
        }
        // Verifica se o evento existe e está disponível para apostas
        const [eventRows] = await mysql_1.default.execute("SELECT * FROM events WHERE id = ? AND status = ?", [eventId, "approved"]);
        ...
        const [walletRows] = await mysql_1.default.execute("SELECT balance FROM wallets WHERE user_id = ?", [user.id]);
        const wallet = walletRows[0];
        if (!wallet || wallet.balance < betAmount) {
            throw new Error("Saldo insuficiente para realizar a aposta.");
        }
        // Registra a aposta
        await mysql_1.default.execute("INSERT INTO bets (user_id, event_id, amount) VALUES (?, ?, ?)", [user.id, eventId, betAmount]);
